...
class UNetInferenceAgent:
    ...
    def single_volume_inference(self, volume):
        ...
        volume_reshaped = med_reshape(volume, new_shape=(volume.shape[0], self.patch_size, self.patch_size))

        for x in range(volume_reshaped.shape[0]):
            volume = torch.from_numpy(volume_reshaped[x] / np.max(volume_reshaped[x])).unsqueeze(0).unsqueeze(0)
            output = self.model(volume.to(self.device, dtype=torch.float))
