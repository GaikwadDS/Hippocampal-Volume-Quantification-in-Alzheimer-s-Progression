...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                        ...
                        Anterior Volume (Red): {inference['anterior']}
                        Posterior Volume (Green): {inference['posterior']}
    ...
    output = np.array(pil_i)
    # Paste the PIL image into our main report image object (pimg)
    pimg.paste(pil_i.resize((400,400)), box=(10, 280))
...
#     fileList = os.listdir(subdirs)
    for fileName in subdirs:
        if (fileName) != ("/data/TestVolumes/Study1/13_HCropVolume"):
            # os.remove(fileName)
    ...
    inference_agent = UNetInferenceAgent(
        device="cpu",
        parameter_file_path=r"section2_out/out/model.pth")
